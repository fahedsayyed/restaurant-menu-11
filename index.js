...
let = dish_name_element = document.getElementById('dish');
document.getElementById('addmore').addEventListener('click',(e)=>{
	e.preventDefault()
	let item = {dish:selecteddish,serving:selectedserving}
	if(!SelecteddishContainer.includes(item)){
		SelecteddishContainer.push({
			dish:selecteddish,
			serving:selectedserving
		})

		for (let index = 0; index < dish_name_element.length; index++) {
			let option = dish_name_element[index];
			// console.log(option.value+'='+item.dish)
			if (option.value==item.dish) {
				dish_name_element[index].remove()
			}
		}


		// dish_name_element.children.map((child,index) => {
		// 	console.log(child.value+"\t"+item.dish);
		// 	if (child.value===item.dish) {
		// 		alert("work");
		// 		dish_name_element.children[index].removeChild()
...
function showOrderDetails(element){
	...
	SelecteddishContainer.map((item) => { text+=item.dish+', '+item.serving+'\n'; })
	console.log(text);
  	document.getElementById('dish_name').innerText 	= text;
